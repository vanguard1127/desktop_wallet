  ...
  describe('currency_format', () => {
    ...
    describe('when the `currencyFrom` option with value "network" is provided', () => {
      ...
      it('should use the i18n locale', () => {
        const amount = Math.pow(10, 4) + 1e-5

        wrapper.vm.$i18n.locale = 'es-ES'
        expect(format(amount, { currencyFrom: 'network' })).toEqual('10.000,00001\xa0×')

        wrapper.vm.$i18n.locale = 'ja-JP'
        expect(format(amount, { currencyFrom: 'network' })).toEqual('×\xa010,000.00001')
      })
    })
    ...
    describe('when the `currencyDisplay` option is provided', () => {
      it('should use it to display the currency', () => {
        const amount = 9835.38765

        expect(format(amount, { currencyFrom: 'network', currencyDisplay: 'code' })).toEqual('NET\xa09,835.38765')
        expect(format(amount, { currencyFrom: 'network', currencyDisplay: 'symbol' })).toEqual('×\xa09,835.38765')
    ...
    describe('when the `subunit` option is provided', () => {
      ...
      describe('when the curreny is not the network currency', () => {
        it('should use it to display the currency, indepently of the `currencyDisplay` option', () => {
          ...
          expect(format(amount, { currencyFrom: 'network', currencyDisplay: 'code', subunit: true })).toEqual('netoshi\xa0983,538,765.00')
          expect(format(amount, { currency: 'NET', currencyDisplay: 'symbol', subunit: true })).toEqual('netoshi\xa0983,538,765.00')
        })
      })
    })
  ...
  describe('subToUnit', () => {
    ...
    describe('when not receiving a network', () => {
      it('should use the session network to convert an amount from arktoshi to ARK', () => {
        ...
        expect(unit.toString()).toEqual('10000000.09801')
      })
    })
